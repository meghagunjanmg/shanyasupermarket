...
const Cart = (props) => {
  ...
  const _getUserDetails = () => {
      ...
      fetch("https://shanya.ca/admin/api/show_address", requestOptions)
  ...
  const deleteHandler = (partOf,desc,index) => {
    ...
    const navigation = props.navigation;
      ...
      await fetch("https://shanya.ca/admin/api/timeslot", requestOptions)
        .then(response => response.json())
        .then(result => {
          console.log(result);
          if(result.status == 0){
              ...
              return(
                  ...
                  <View style={{flexDirection:"row"}}>
								  	<TouchableOpacity onPress={() => {/*this.props.navigation.navigate('ProductDetails', {productDetails: item})*/}} style={{padding: 10,flex: 1,}}>
								  		<Image source={{uri: 'https://shanya.ca/admin/' + item.product_image}} style={{height:80,width:80}}/>
								  	</TouchableOpacity>
